...
export function NetworkProvider({ children }: { children: React.ReactNode }) {
      ...
      if (slot && slot.status === 'matched' && (slot.hostUid === uid || slot.guestUid === uid)) {
        ...
        if (isMeHost) {
          await update(ref(db, `rooms/${slot.matchId}`), {
            id: slot.matchId, stake, hostUid: uid, hostHeartbeatAt: serverTimestamp(),
            participants: [{ uid: slot.hostUid, name: slot.hostName }, { uid: slot.guestUid, name: slot.guestName }],
      ...
      try {
        await runTransaction(waitingSlotRef, current => {
          ...
          if (current.status === 'waiting') {
            ...
            if (current.hostUid !== uid && (!current.preferredGuestUid || isPreferredGuest || isStale)) {
  ...
  // Listen at the smallest useful paths. A room-level onValue caused every
  // ball snapshot, heartbeat and aim update to download the complete room.
  useEffect(() => {
    ...
    const onState = onValue(stateRef, snap => {
      const raw = snap.val();
      if (!raw) { roomDataRef.current.state = null; setGameState(null); return; }
    ...
    const onBalls = onValue(ballsRef, snap => setBalls(snap.val() || null));
    ...
    const onHeartbeat = onValue(heartbeatRef, snap => { const v = snap.val(); if (typeof v === 'number') hostHeartbeatRef.current = v; });
    return () => {
      off(stateRef, 'value', onState); off(ballsRef, 'value', onBalls); off(aimRef, 'value', onAim);
      off(rematchRef, 'value', onRematch); off(hostRef, 'value', onHost); off(heartbeatRef, 'value', onHeartbeat);
    };
  }, [roomId]);

  useEffect(() => {
    if (!roomId || !currentPlayerUid || !isFirebaseConfigured) { currentPresenceRef.current = null; return; }
  ...
  useEffect(() => {
    ...
    const db = getRtdb();
    const interval = setInterval(async () => {
      const state = roomDataRef.current.state;
      ...
      await runTransaction(ref(db, `rooms/${roomId}/state`), (currentState: HUDState | null) => {
        ...
        let next = (idx + 1) % players.length, tries = 0;
        while (players[next]?.isBenched && tries < players.length) { next = (next + 1) % players.length; tries++; }
        return { ...currentState, players, currentPlayerIndex: next, phase: 'aiming', timeLeft: 60, power: 0, aimAngle: 0, spin: { x: 0, z: 0 }, shotResult: { type: 'foul_wrong', pottedBalls: [], scoreChange: 0, message: `⚠ ${players[idx].name} timed out/offline — turn forfeited`, extraTurn: false } as any };
      }).catch(() => {});
    }, 1500);
    return () => clearInterval(interval);
  ...
  const updateAuthoritativeState = useCallback((state: HUDState, balls: BallState[]) => {
    ...
    update(ref(getRtdb(), `rooms/${roomId}`), {
      state, balls: balls || [], hostUid: user?.uid || null,
      hostHeartbeatAt: serverTimestamp(), updatedAt: serverTimestamp()
    }).catch(() => {});
  }, [roomId, user?.uid]);
  ...
  const sendAimState = useCallback((aimAngle: number, power: number, spin: Vec2, pos?: Vec2) => {
    if (!roomId) return;
    update(ref(getRtdb(), `rooms/${roomId}/activeAim`), { aimAngle, power, spin, pos: pos || null, updatedAt: serverTimestamp(), uid: user?.uid }).catch(() => {});
  }, [roomId, user?.uid]);
  ...
  const resetRoom = useCallback((players: any[], stake: number, previousScores?: Record<string, number>) => {
    ...
    update(ref(getRtdb(), `rooms/${roomId}`), {
      intents: null, activeAim: null, rematchVotes: null, previousScores: previousScores || null,
      stake, hostUid: user?.uid || null, hostHeartbeatAt: serverTimestamp()
  ...
  const leaveRoom = useCallback(() => {
    if (roomId && isHost) {
      const db = getRtdb();
      setTimeout(() => remove(ref(db, `rooms/${roomId}`)).catch(() => {}), 3000);
    }
    setRoomId(null); setGameState(null); setBalls(null); setActiveAim(null); setIsHost(false);
  }, [roomId, isHost]);

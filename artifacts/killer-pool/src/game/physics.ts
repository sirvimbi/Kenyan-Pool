...
// Coefficients are converted from a 60-Hz reference to the deterministic
// fixed timestep so refresh rate does not change the physical trajectory.
...
export function stepPhysics(
  ...
  for (const b of active) {
    ...
    if (b.spin) {
      // Top/back spin transfers energy gradually while the ball is moving.
      // It is deliberately disabled near rest to avoid artificial acceleration.
      if (speed > 0.08 && Math.abs(b.spin.z) > 0.001) {
        const dirX = b.vel.x / speed;
        const dirZ = b.vel.z / speed;
        const transfer = b.spin.z * 0.045 * fixedDt * 60;
    ...
    if (speed > MIN_SPEED) {
      ...
      // Only settle the final few cm/s. The previous larger cutoff caused a
      // visible snap at the end of a roll.
      b.vel.x = 0;
      b.vel.z = 0;
      if (b.spin) {
        b.spin.x *= 0.90;
  ...
  for (let i = 0; i < active.length - 1; i++) {
    for (let j = i + 1; j < active.length; j++) {
      ...
      // Small, capped tangential transfer gives English a believable effect on
      // the object ball without creating energy or a new launch direction.
      ...
      if (cue?.spin && object) {
        ...
        object.vel.x += -nz * tangential;
        object.vel.z += nx * tangential;
        cue.spin.x *= 0.96;
        cue.spin.z *= 0.94;
      }

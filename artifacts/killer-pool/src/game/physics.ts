 ...
 * Must be called with a constant dt for determinism.
 */
export function stepPhysics(
  ...
  // 1. Integration (Movement & Friction)
  // pow(FRICTION, fixedDt * 60) is constant for a fixed step
  ...
  // 2. Boundary Collisions (Cushions)
  for (const b of active) {
    let bounced = false;
    if (b.pos.x < -HW) {
      b.pos.x = -HW;
  ...
  // 3. Object-to-Object Collisions (Balls)
  const minD = BALL_R * 2;

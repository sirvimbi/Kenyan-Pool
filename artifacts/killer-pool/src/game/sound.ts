...
export const sound = {
  ...
  cueStrike(power = 0.5) {
    ...
    ns.connect(bpf); bpf.connect(ng); ng.connect(a.destination);
    ns.start(t);
  ...
  ballClick(speed = 3) {
    ...
    ns.connect(hpf); hpf.connect(ng); ng.connect(a.destination);
    ns.start(t);
  },
  ...
  pocketDrop() {
    ...
    ns.connect(lpf); lpf.connect(ng); ng.connect(a.destination);
    ns.start(t + 0.05);
